...
class M_jurnalumum extends CI_Model
{
    ...
    public function selectingData($select)
    {
        if ($select == 'all') {
            return $this->db->get('jurnalumum')->result_array();
        } else if ($select == 'up') {
            ...
            return $this->db->get_where('jurnalumum', array('tgl' => $select, 'tutup_buku' => 'Y'))->result_array();
        }
    }
    ...
    public function getTransaksi($teks)
    {
        $this->db->select('*');
        $this->db->like('transaksi', $teks);
        return $this->db->get('jurnalumum')->result_array();
    }
